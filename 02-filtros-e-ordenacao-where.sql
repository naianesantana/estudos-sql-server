...
SELECT 
	...
	UnitPrice,
	Weight,
	AvaliableForSaleDate
FROM
	DimProduct
ORDER BY 
	UnitPrice DESC,
	Weight DESC,
	AvalaiableForSaleDate ASC

-- [CERTO] -- Exercício 3: Você é responsável pelo setor de logística da empresa Contoso e precisa dimensionar o transporte de todos os produtos em categorias, de acordo com o peso. Os produtos da categoria A, com peso acima de 100kg, deverão ser transportados na primeira leva.
...
-- [CERTO] -- Exercício 4: Você foi alocado para criar um relatório das lojas registradas atualmente na Contoso.
...
SELECT COUNT(StoreKey) AS 'Total de Lojas' FROM DimStore
...
WHERE 
	...
	AND AvailableForSaleDate = '20090315' -- [IMPORTANTE: Opção de formatação de data para tipos DateTime]

-- [CERTO] -- Exercício 6: Imagine que você precise extrair um relatório da tabela DimStore, com informações de lojas. Mas você precisa apenas das lojas que não estão mais funcionando atualmente.
...
-- [CERTO] -- Exercício 7: De acordo com a quantidade de funcionários, cada loja receberá uma determinada quantidade de máquinas de café. As lojas serão divididas em 3 categorias:
...
-- [CERTO] -- Exercício 8: A empresa decidiu que todas as televisões de LCD receberão um super desconto no próximo mês. O seu trabalho é fazer uma consulta à tabela DimProduct e retornar os ID’s, Nomes e Preços de todos os produtos LCD existentes.
...
-- [CERTO] -- Exercício 9: Faça uma lista com todos os produtos das cores: Green, Orange, Black, Silver e Pink. Estes produtos devem ser exclusivamente das marcas: Contoso, Litware e Fabrikam.
...
-- [CERTO] -- Exercício 10 (Deu 15 produtos, mas o enunciado diz que são 16. No vídeo da correção, o instrutor utilizou o parâmtero Weight entre 10 e 30 ao invés do UnitPrice, como descreve o enunciado do exercício): A empresa possui 16 produtos da marca Contoso, da cor Silver e com um UnitPrice entre 10 e 30. Descubra quais são esses produtos e ordene o resultado em ordem decrescente de acordo com o preço (UnitPrice).
